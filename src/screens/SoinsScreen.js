            ...
            <>
              ...
              {activeView === 'urgencies' && (
                <View style={{ padding: 20, borderRadius: 24, backgroundColor: isDark ? '#121212' : brandColor + '10', borderWidth: 1, borderColor: isDark ? '#1E1E1E' : brandColor + '30', marginBottom: 24 }}>
                  ...
                  <TextInput style={{ backgroundColor: isDark ? '#1A1A1A' : '#FFF', color: isDark ? '#FFF' : '#0A0A0A', padding: 12, borderRadius: 12, borderWidth: 1, borderColor: isDark ? '#2E2E2E' : '#E2E8F0', marginBottom: 12 }} placeholder={(typeof t !== 'undefined' && t.dynamic ? t.dynamic : {})["Motif de l"] || "Motif de l"}urgence (Ex: Saignements, Inconscient...)" placeholderTextColor={isDark ? '#888888' : '#94A3B8'} value={emergencyForm.complaints} onChangeText={v => setEmergencyForm(prev => ({ ...prev, complaints: v }))} />
                  ...
                  <View style={{ marginTop: 8 }}>
                    <TextInput
                      ...
                      onChangeText={q => {
                        setMedSearch(q);
                        if (q.length > 1) {
                           setMedResults(medicines.filter(m => m.name.toLowerCase().includes(q.toLowerCase())).slice(0, 5));
                        } else setMedResults([]);
                      }}
                    />
                    {medResults.length > 0 && (
